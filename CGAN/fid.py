...
def get_fid(images1, images2):
    ...
    assert (np.min(images1[0]) >= 0 and np.max(images1[0]) < 256), 'Image values should be in the range [0, 255]'
    assert (type(images2) == np.ndarray)
    assert (len(images2.shape) == 4)
    assert (images2.shape[1] == 3)
    assert (np.min(images2[0]) >= 0 and np.max(images2[0]) < 256), 'Image values should be in the range [0, 255]'
