...
def load_patches(save_folder, s, thresh=0.8):
    ...
    - thresh: treshold.
    OUTPUT:
    - loaded_patches: patches of dim. (n_patches, dxd, n_channels)
...
class CGAN():
    def __init__(self, filepath, replay=True, replay_start_record=5000, replay_start_substitution=6000, replay_interval=100,
                 replay_examples=1, replay_file=True, replay_proportion=0.3, latent_dim=100, num_classes=2):
        """
        INPUT:
        - filepath: path where the models and images will be saved,
        - replay: bool, must be True to save images.
        - replay_start_record: the batch to begin saving samples,
        - replay_start_substitution: the batch to begin substituting samples,
        - replay_interval: interval to save/substitute samples,
        - replay_examples: number of samples to save at each interval,
        - replay_file: bool, 
                    if True: saves the datasets in a hdf5 file.
                    if False: saves the datasets in lists.
        - replay_proportion: ratio of historical/current samples,
        ...
        self.combined = Model([noise, label, label_tensor], valid) # Model(inputs, outputs)
    ...
    def build_deep_generator(self):
        ...
        model.add(UpSampling2D(interpolation='bilinear'))  # size: (4*d1,4*d1,256)=32*32*256 


        model2 = Sequential()  # size: (img_size/2,img_size/2,256+num_classes)
        model2.add(Conv2D(128, kernel_size=3, padding="same",  # size: (self.img_rows/2,self.img_cols/2,,128)
                          input_shape=(int(self.img_rows/2), int(self.img_cols/2), 256 + self.num_classes), kernel_initializer='he_normal'))
        ...
        model2.add(Conv2D(self.channels, kernel_size=3, padding='same'))  # size: (img_size,img_size,self.channels)
        ...
        label_tensor = Input(shape=(int(self.img_rows/2), int(self.img_cols/2), self.num_classes), dtype='float32')

        label_embedding = Flatten()(Embedding(self.num_classes, self.latent_dim)(label))
        model_input = multiply([noise, label_embedding])
        r = model(model_input)
        # padding to match the dimensions: 32x32 -> 35x35
        padding = tf.constant([[0,0], [2,1], [2,1], [0,0]]) # only pad around the OCT images
        r_pad = tf.pad(r, padding, 'SYMMETRIC')
        merged = concatenate([r_pad, label_tensor])

        img = model2(merged)

        return Model([noise, label, label_tensor], img) # Model(inputs, outputs)

    def build_discriminator(self):
        model = Sequential()

        # add a first block to reduce the size of the image from 70x70 to 32x32
        model.add(Conv2D(32*32, kernel_size=6, strides=2, input_shape=self.img_shape, padding='valid')) #no padding
        model.add(BatchNormalization(momentum=0.8))
        model.add(LeakyReLU(alpha=0.2))

        model.add(Conv2D(32, kernel_size=3, strides=1, input_shape=(32, 32, self.channels), padding="same"))
        model.add(LeakyReLU(alpha=0.2))
        ...
        model_input = multiply([img, label_embedding]) #of shape img_shape

        validity = model(model_input)

        return Model([img, label], validity) # Model(inputs, outputs)
    ...
    def train(self, epochs, batch_size=128, sample_interval=50, model_interval=1000, stride=2, thresh = 0.8, smooth_type="onesided"):
        ...
        - sample_interval: interval in which images are saved,
        - model_interval: interval in which the model is saved,
        - stride: stride in between the patches from the original OCT images,
        - smooth_type: if 'onesided', performs 'onesided' label smoothing. 
        No outputs.
        """
        ...
        # create_dataset: very similar to Numpy arrays.
        ...
        for i in range(r):
            for j in range(c):
                axs[i,j].imshow(np.transpose(X_train[cnt*100,:,:,0]), cmap='gray', vmin=0, vmax=1)
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig(self.filepath + "/loaded_patches.png")
        plt.close()
        ...
        for epoch in range(epochs):
            ...
            sampled_labels = np.random.randint(0, self.num_classes, (batch_size, 1)) # (low, high, size)
            ...
            d_loss_real = self.discriminator.train_on_batch([imgs, img_labels], valid) #train_on_batch: runs a single gradient update on a single batch of data, (x, y)
    ...
    def sample_images(self, epoch):
        """
        sample r*c images from the generator and saves them at self.filepath.
        Input:
        - epoch: epoch at which the images are being sampled, is used to be included in the name of the file.
        """
    ...
    def save_model(self, epoch):
        """
        Saves the model in self.filepath.
        Input:
        - epoch: epoch at which the model is, is used to be included in the name of the file.
        """
    ...
    def load_data(self, stride, thresh):
        ...
        - y_train: labels.
        """
        x_train, y_train = load_patches("/data/line/OCT-patchbased-CGAN/generated_patches/training", stride, thresh)
        return x_train, y_train

    def label_smoothing(self, vector, max_dev=0.2, smooth_type="onesided"):
        """ 
        'onesided' label smoothing is employed to help prevent overconfidence of the discriminator. 
        It is done here by setting each real label to some random number (uniformly distributed) between 0.8 and 1.
        If label 1: label will be randomly asigned to a value between 0.8 and 1.
        If label 0: 
            - if 'onesided: label stays 0.
            - else: label will be randomly asigned to a value between 0. and 0.2.
        """
...
if __name__ == '__main__':
    """ default:
    cgan = CGAN(filepath="cgan_70x70_patchbased", replay=True, replay_start_record=5000, replay_start_substitution=6000, replay_interval=100,
                replay_examples=1, replay_file=True, latent_dim=100, num_classes=10)
    cgan.train(epochs=1000000, batch_size=64, sample_interval=100, model_interval=1000, stride = 2, smooth_type="onesided")
    """
    # default variables
    stride = 4
    thresh = 0.8
    ...
    cgan = CGAN(filepath="/data/line/OCT-patchbased-CGAN/cgan_70x70_patchbased/"+'thresh='+ str(int(thresh*100))+"/s="+str(stride), replay=True, replay_start_record=5000, replay_start_substitution=6000, replay_interval=100,
                replay_examples=1, replay_file=True, latent_dim=100, num_classes=2)
    cgan.train(epochs=10000, batch_size=64, sample_interval=100, model_interval=1000, stride=stride, thresh=thresh, smooth_type="onesided")
